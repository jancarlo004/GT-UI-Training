...
const summaryScreen = new Summary();
export class AccountScenarios {
  ...
  async createNewAccount() {
    ...
    if(await accountTabBar.searchAccount.component.find('div.gw-ListViewWidget[id$=-NewAccountSearchResultsLV]').exists == true){
      ...
      await t.pressKey('tab');
      await helper.selectDropdownValue('CreateAccount-CreateAccountScreen-CreateAccountDV-AddressType','home');
  ...
  async getAccoutInformation(){
    world.accountInformation.accountNumber = await summaryScreen.accountDetailsDetailViewTile_DVAccountNumber.getValue();
    world.accountInformation.accountName = world.newAccount_Contact.fullName;
    console.log(`Account Number: ${world.accountInformation.accountNumber} \nAccount Name: ${world.accountInformation.accountName}` );
  }
}