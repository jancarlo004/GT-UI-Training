...
export class AccountCreationScenario{
    ...
    async createCompanyAccount(){
        ...
        if( await newAccount.newAccountScreenNewAccountSearchResultsLV.rowCount() != 0){
            ...
            await createAccountWizard.primaryEmailTextInput.setValue(world.CompanyInsuredInfo.CompanyInfo.companyName.split(" ").join("")+"@Test.Test");
            await createAccountWizard.phoneNumber.setValue(await autoGeneratedCustomMethod.generateHomePhoneNumber());
    ...
    async accountCreationValidation(name){

        let verifiedWords = 'Account Summary: '+name;

        let textToVerified = await accountTabBar.tabBarAccountTab.component.parent().sibling().find('.gw-TitleBar--title').innerText;

        await t.expect(verifiedWords).contains(textToVerified, 'Unsuccessfull Account Creation');
